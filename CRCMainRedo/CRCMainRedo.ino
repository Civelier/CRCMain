...
struct MotorGroup
{
	...
	void MoveDistance(double deg, double speed)
	{
		SpeedPID.limit(-speed, speed);
		MaxSpeed = speed;
	...
	void TurnDistance(double deg, double speed)
	{
		SpeedPID.limit(-speed, speed);
		MaxSpeed = speed;
...
void SetupDriving()
{
	motorL = new Bilda5202Motor(CRC_PWM_1, CRC_ENCO_A, CRC_ENCO_B);
	motorR = new Bilda5202Motor(CRC_PWM_2, CRC_SERIAL_RXD1, CRC_SERIAL_TXD1, true);
	driving = new MotorGroup(motorL, motorR);
	driving->Begin();
}
#define SETUP_DRIVING() SetupDriving()
#else
#define SETUP_DRIVING() 
#endif
